...
assert devenv.inputs.nixd.inputs.treefmt-nix.rev == self.unified.treefmt-nix.rev;
...
assert devenv.inputs.cachix.inputs.git-hooks.rev == self.pinned.cachix.inputs.git-hooks.rev;
